"""Train XGBoost drought prediction models."""
...
try:
    ...
    raise SystemExit("xgboost is required. Rebuild the backend image after updating requirements.") from exc
...
def build_xgb_model(
    *,
    ...
    return XGBRegressor(
        ...
        subsample=min(max(float(subsample), 0.4), 1.0),
        colsample_bytree=min(max(float(colsample_bytree), 0.4), 1.0),
        objective="reg:squarederror",
        n_jobs=max(1, int(os.getenv("PREDICTION_XGBOOST_JOBS", "2"))),
        random_state=42,
    )
...
def backtest_recursive_xgb(
    ...
    for group_idx, (feature_id, group) in enumerate(groups, start=1):
        if group_idx == 1 or group_idx % 10 == 0 or group_idx == total_groups:
            log_progress(f"[backtest] features {group_idx}/{total_groups} = {(100.0 * group_idx / total_groups):0.1f}%")
        ...
        for origin in origins["date"].iloc[::3]:
            ...
            if len(hist) < input_window:
                continue
            forecasts = recursive_forecast_xgb(model, hist, feature_columns, horizon, climatology)
            actual_map = {
...
def train_group(
    *,
    ...
    if len(train_x) < 32:
        ...
        return
    log_progress(
        f"[{source_key}/{index}/{METHOD_NAME}] fitting evaluation booster on "
        f"{len(train_x):,} rows | features={train_x.shape[1]} | trees={max(50, int(n_estimators))}"
    )

    eval_model = build_xgb_model(
    ...
    log_progress(f"[{source_key}/{index}/{METHOD_NAME}] evaluation booster fit complete")
    ...
    for ds_idx, ds in enumerate(datasets, start=1):
        ...
        if frame is None:
            continue
        log_progress(f"[{source_key}/{index}/{METHOD_NAME}] backtesting dataset {ds_idx}/{len(datasets)}: {ds.key}")
        bt = backtest_recursive_xgb(eval_model, frame, feature_columns, input_window, horizon, climatology, cutoff)
        if not bt.empty:
            bt["dataset_key"] = ds.key
            all_eval_rows.append(bt)
        log_progress(f"[{source_key}/{index}/{METHOD_NAME}] backtest done for {ds.key}: rows={len(bt):,}")
    ...
    log_progress(
        f"[{source_key}/{index}/{METHOD_NAME}] fitting final booster on "
    ...
    log_progress(f"[{source_key}/{index}/{METHOD_NAME}] final booster fit complete")
    ...
    for ds_idx, ds in enumerate(datasets, start=1):
        ...
        for group_idx, (feature_id, group) in enumerate(groups, start=1):
            ...
            preds = recursive_forecast_xgb(final_model, group, feature_columns, horizon, climatology)
            for item in preds:
        ...
        log_progress(f"[{source_key}/{index}/{METHOD_NAME}] forecast done for {ds.key}: rows={len(forecast_rows):,}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train XGBoost drought prediction models")
